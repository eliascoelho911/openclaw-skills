...
def test_monthly_report_returns_transfer_instruction_for_populated_month(
    ...
    assert (
        client.post(
            ...
            json={
                ...
                "requested_by_participant_id": participant_a,
                "original_purchase_external_id": "wpp-301",
            },
        ).status_code
        == 201
    )
...
def test_monthly_report_returns_no_transfer_for_empty_month(
    client: TestClient, participants: tuple[str, str]
) -> None:
    participant_a, participant_b = participants
...
def test_monthly_report_auto_generate_is_idempotent(
    client: TestClient, participants: tuple[str, str]
) -> None:
    ...
    first_response = client.get("/v1/months/2026/2/report?auto_generate=true")
    assert first_response.status_code == 200
    first_body = first_response.json()
    assert first_body["total_gross"] == "120.00"
    assert first_body["total_net"] == "120.00"

    second_response = client.get("/v1/months/2026/2/report?auto_generate=true")

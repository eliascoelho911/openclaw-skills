...
- Q: O mês deve ser fechado na v1? → A: Não; o mês permanece aberto e o relatório mensal é sempre gerado sob demanda.
...
**Why this priority**: Sem registro confiável das movimentações, não existe base para calcular quem deve transferir para quem em cada mês.
...
**Why this priority**: Esse acompanhamento reduz surpresas na conciliacao mensal e permite corrigir erros de lançamento antes do fim do mês.
...
### User Story 3 - Gerar relatório mensal sob demanda (Priority: P3)

Como participante do casal, quero gerar um relatório mensal sob demanda para ter uma instrução objetiva de quem deve transferir quanto ao outro.

**Why this priority**: O relatório sob demanda automatiza a etapa hoje manual e reduz erro humano sem exigir bloqueio do mês.

**Independent Test**: Pode ser testado de forma independente consultando um mês com movimentações conhecidas e verificando se o relatório contém os valores esperados e a instrução de transferência.

**Acceptance Scenarios**:

1. **Given** um mês com movimentações válidas, **When** o relatório mensal é solicitado, **Then** o sistema gera um resumo consolidado com a instrução de transferência entre os participantes.
2. **Given** um mês sem movimentações, **When** o relatório mensal é solicitado, **Then** o sistema retorna totais zerados e informa ausência de transferência pendente.
...
- Relatório de mês sem movimentações deve retornar saldo zero e sem transferência.
...
- Como não há fechamento na v1, novos lançamentos em um mês já consultado devem atualizar automaticamente o relatório em consultas futuras.
...
- **FR-007**: O sistema MUST disponibilizar relatório mensal sob demanda contendo período, movimentações consolidadas, total líquido do mês e instrução explícita de transferência.
- **FR-008**: O sistema MUST manter histórico consultável de movimentações por, no mínimo, 24 meses.
- **FR-009**: O sistema MUST registrar trilha de auditoria mínima para cada movimentação e consulta de relatório (identificador do participante informado na requisição e timestamp).
- **FR-010**: O sistema MUST exibir valores monetários em Real brasileiro com duas casas decimais em todos os resumos e relatórios.
- **FR-011**: A API v1 MUST operar sem autenticação/autorização; ao registrar movimentações e consultar relatórios, a requisição MUST informar explicitamente o identificador do participante responsável quando aplicável.
- **FR-012**: O sistema MUST adotar histórico append-only para movimentações: não permitir edição nem exclusão; correções MUST ocorrer por estorno vinculado e novo lançamento, quando necessário.
- **FR-013**: O sistema MUST arredondar cada movimentação para duas casas decimais (BRL) no momento do registro e utilizar esses valores arredondados para todos os cálculos acumulados de resumo e relatório.
- **FR-014**: O sistema MUST rejeitar como duplicada qualquer nova movimentação que repita o mesmo identificador externo para o mesmo participante dentro do mesmo mês de competência.
- **FR-015**: O sistema MUST determinar o mês de competência pela data/hora da movimentação no fuso fixo America/Sao_Paulo, usando o timestamp atual do registro quando a data não for informada.
- **FR-016**: Quando `payer_participant_id` não for informado, o sistema MUST assumir automaticamente o mesmo valor de `requested_by_participant_id`.
- **FR-017**: Ao receber estorno com `original_purchase_external_id`, o sistema MUST resolver a compra original por contexto de competência mensal e participante pagador efetivo; se não encontrar correspondência única, MUST rejeitar com erro claro.
...
- **Relatório Mensal**: Resultado consolidado de um mês calculado sob demanda, com indicação de devedor/credor e valor de transferência.
...
- **DEPENDENCY-001**: Os dois participantes precisam estar previamente cadastrados e ativos para registrar movimentações e consultar relatórios do mês.
...
- **UX-001**: A terminologia exibida ao usuário MUST ser consistente em todos os fluxos: "compra", "estorno", "relatório", "saldo" e "transferência".
- **UX-002**: Mensagens de validação e erro MUST indicar causa do problema e ação de correção em linguagem simples.
- **UX-003**: O cálculo e os rótulos de saldo MUST permanecer idênticos entre resumo parcial e relatório mensal, evitando interpretações conflitantes.
...
- **PR-003**: A geração de relatório mensal sob demanda MUST concluir em até 5 segundos para meses com até 5.000 movimentações.
- **PR-004**: A validação de desempenho MUST ser executada antes da liberação com testes de carga representativos dos cenários de registro, consulta de resumo, busca de movimentações e geração de relatório mensal.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: Em 100% dos meses consultados, o relatório mensal informa claramente quem deve transferir, para quem e o valor exato.
...
- **SC-003**: Para meses com até 200 movimentações, o processo completo de consulta e conferência do relatório mensal é concluído em até 5 minutos por um participante.

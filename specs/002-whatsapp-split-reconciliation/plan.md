...
fechar o mes com relatorio final idempotente, incluindo busca de movimentacoes
por filtros e suporte a estorno vinculado por `original_purchase_external_id`.
O desenho tecnico adota SQLAlchemy 2.x + psycopg para acesso ao banco, Alembic
para migracoes, calculo monetario com `Decimal` (2 casas) e regras de dominio
para deduplicacao, limite de estorno e imutabilidade apos fechamento.

## Technical Context

**Language/Version**: Python 3.12  
**Primary Dependencies**: FastAPI, Pydantic v2, SQLAlchemy 2.x, psycopg 3, Alembic, Typer  
**Storage**: PostgreSQL 16 (movimentacoes append-only e fechamento mensal imutavel)  
...
- **UX Consistency Gate**: PASS - a API mantera terminologia unica (`compra`, `estorno`, `fechamento`, `saldo`, `transferencia`) e erros com causa + acao esperada.
...
- **Simplicity & Safety Gate**: PASS - abordagem simples (CRUD restrito + fechamento idempotente), com trilha de auditoria minima e logs estruturados para fluxos criticos.
...
- **UX Consistency Gate**: PASS - contratos padronizam dinheiro BRL com duas casas e formato de datas consistente em resumo e fechamento.
...
- **Simplicity & Safety Gate**: PASS - sem violacoes constitucionais; complexidade extra (idempotencia + bloqueio para estorno) e necessaria para FR-004, FR-009 e FR-016.
